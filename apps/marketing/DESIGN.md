...
The lockup is **"SoldDirect."** — one word, bold, with the "o" in "Sold" drawn
as a green speech bubble (the WhatsApp nod) and a trailing full stop. The
wordmark is **outlined vector art, never typeset**: the exact paths live in
`components/logo-paths.ts` (extracted from the master; regenerate from the .ai
if the logo ever changes, don't edit by hand).

**The ™ is currently off.** It shipped briefly and was removed by decision —
the lockup carries no trade mark symbol for now. The CIPC application is
unaffected; `context/trade-mark.md` still tracks where it stands. To put the
™ back, restore `TRADEMARK_PATH` and `LOGO_VIEWBOX_TM` in `logo-paths.ts`
(see the history of that file), draw the extra path in `Logo.tsx`, rebuild
`public/logo.svg`, and regenerate the signature PNG. Whenever it returns it
must be ™ and never **®**, which may not be used until a registration
certificate exists.
...
- No trade mark symbol on the lockup for now — see above. If one is ever
  reinstated it is ™, never ®, until a registration certificate exists:
